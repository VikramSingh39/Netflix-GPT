  ...
  const handleLanguageChange = (e)=>{
    ...
    return(
        ...
        <div className='flex absolute justify-between z-50 w-full px-4 py-4 bg-gradient-to-b from-black flex-col md:flex-row whitespace-nowrap'>
           <img className='w-34 cursor-pointer mx-auto md:mx-0' src={logo}/>
 
          {user && (<div className='flex flex-wrap justify-between  items-center gap-2'>
          


            <button className="p-1 md:p-2 bg-white rounded cursor-pointer" onClick={handleTmdbSearchClick}> {showTmdbSearch ? "Go to Home": "Search With TMDB"}</button>
            <img className='w-[40px] h-[40px] rounded cursor-pointer' src={userProfile} alt="" />
            <button className='p-1 md:p-2 bg-white rounded cursor-pointer' onClick={handleSignOut}>Sign Out</button>
           </div>)}
          {
